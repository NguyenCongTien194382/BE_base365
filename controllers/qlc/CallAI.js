...
exports.DetectFace = async (req, res) => {
  ...
  try {
    if (comp_id && image) {
      let resp
      if (Number(comp_id) > 10000000 || Number(comp_id) === 1664 || Number(comp_id) === 2222) {
        if (user_id) {
...
exports.DataThongKeGiaLap = async (req, res, next) => {
  try {
    ...
    if (hinh_thuc_truy_cap) formDataCom.append('hinh_thuc_truy_cap', hinh_thuc_truy_cap)
