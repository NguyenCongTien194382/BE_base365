...
exports.editNhom = async(req, res) => {
    try {
        ...
        let chinhsuanhom = await NhomTaiSan.findOneAndUpdate({ id_nhom: id_nhom, id_cty: com_id, nhom_da_xoa: 0 }, { $set: { ten_nhom: ten_nhom } }, { new: true });
        if (!chinhsuanhom) {
...
exports.xoaNhom = async(req, res) => {
    try {
        ...
        if (type == 2) {
            ...
            let result = await NhomTaiSan.updateMany({ id_nhom: { $in: idArray }, nhom_da_xoa: 0, id_cty: com_id }, {
                nhom_da_xoa: 1,
...
exports.listTTPBnhom = async(req, res) => {
    try {
        ...
        let checkTTTC = await ThongTinTuyChinh.find(query)
            .skip(startIndex)
            .limit(perPage)
            .select('id_tt tt_ten_truong kieu_du_lieu noidung_mota ng_tao tt_date_create');

        const items = await Promise.all(checkTTTC.map(async(tt) => {
            let nguoi_tao = '';
            let checkUser = await User.findOne({ _id: tt.ng_tao, type: { $ne: 0 } }).select('userName');
            if (checkUser) {
...
exports.editTTTC = async(req, res) => {
    try {
        ...
        let chinhsua = await ThongTinTuyChinh.findOneAndUpdate({ id_tt: id_tt, com_id_tt: com_id, tt_xoa: 0 }, {
            $set: {
                tt_ten_truong: tt_ten_truong,
                kieu_du_lieu: kieu_du_lieu,
                noidung_mota: noidung_mota
...
exports.deleteTTTC = async(req, res) => {
    try {
        ...
        if (type == 2) {
            // thay đổi trang thái thành 1
            let result = await ThongTinTuyChinh.findOneAndUpdate({ id_tt: id_tt, tt_xoa: 0, com_id_tt: com_id }, {
                tt_xoa: 1,
                ng_xoa: ng_xoa,
                ngay_xoa: deleteDate
            });
