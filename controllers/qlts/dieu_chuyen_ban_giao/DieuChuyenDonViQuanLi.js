...
exports.list = async (req, res, next) => {
  try {
    ...
    if (id_nv) filter["$or"] = [
      { "id_ng_thuchien": Number(id_nv) },
      { "id_daidien_nhan": Number(id_nv) },
      { "id_nv_nhan": Number(id_nv) },
    ]
    ...
    if (type == 1) {
      ...
      let data = await DieuChuyen.aggregate([
...

        {
          $project: {
            ...
            pb_ng_nhan: "$daidien_nhan.inForPerson.employee.organizeDetailId",
            ten_ng_nhan: '$daidien_nhan.userName',
            pb_ng_thuc_hien: "$users_id_ng_thuchien.inForPerson.employee.organizeDetailId",
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (data[i].id_pb_dang_sd != 0) {
          ...
          if (id_pb_dang_sd) data[i].name_pb_dang_sd = id_pb_dang_sd.organizeDetailName
        ...
        if (data[i].pb_ng_thuc_hien != 0) {
          let depName_nguoi_thuchien = await OrganizeDetail.findOne({ id: data[i].pb_ng_thuc_hien }, { organizeDetailName: 1 })
          if (depName_nguoi_thuchien) data[i].depName_nguoi_thuchien = depName_nguoi_thuchien.organizeDetailName
          else data[i].depName_nguoi_thuchien = "Chưa cập nhật"

        }
        if (data[i].pb_ng_nhan != 0) {
          let depName_ng_nhan = await OrganizeDetail.findOne({ id: data[i].pb_ng_nhan }, { organizeDetailName: 1 })
          if (depName_ng_nhan) data[i].depName_ng_nhan = depName_ng_nhan.organizeDetailName
          else data[i].depName_ng_nhan = "Chưa cập nhật"
        }
        ...
        if (data[i].id_pb_nhan != 0) {
          ...
          else data[i].id_pb_nhan = "Chưa cập nhật"
        }
    ...
    if (type == 2) {
      ...
      let data = await DieuChuyen.aggregate([
        ...
        { $unwind: { path: "$daidien_nhan", preserveNullAndEmptyArrays: true } },
        {
          $project: {
            ...
            pb_daidien_nhan: '$users_id_ng_daidien.inForPerson.employee.organizeDetailId',
            ten_ng_nhan: '$daidien_nhan.userName',
            pb_ng_nhan: "$daidien_nhan.inForPerson.employee.organizeDetailId",
            ten_ng_thuchien: '$users_id_ng_thuchien.userName',
            pb_ng_thuc_hien: "$users_id_ng_thuchien.inForPerson.employee.organizeDetailId",
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (data[i].pb_daidien_nhan != 0) {
            let depName_ng_nhan = await OrganizeDetail.findOne({ id: data[i].pb_daidien_nhan }, { organizeDetailName: 1 })
            if (depName_ng_nhan) data[i].depName_ng_daidien_nhan = depName_ng_nhan.organizeDetailName
            else data[i].depName_ng_daidien_nhan = "Chưa cập nhật"
          }
        ...
        if (data[i].id_pb_nhan != 0) {
          let id_pb_nhan = await OrganizeDetail.findOne({ id: data[i].id_pb_nhan }, { organizeDetailName: 1 })
          if (id_pb_nhan) data[i].name_pb_nhan = id_pb_nhan.organizeDetailName
          else data[i].name_pb_nhan = "Chưa cập nhật"
        }
        if (data[i].dc_ngay != 0) data[i].dc_ngay = new Date(data[i].dc_ngay * 1000);
        if (data[i].dc_hoan_thanh != 0) data[i].dc_hoan_thanh = new Date(data[i].dc_hoan_thanh * 1000);
        if (data[i].dc_date_create != 0) data[i].dc_date_create = new Date(data[i].dc_date_create * 1000);
...
exports.excelDCVT = async (req, res, next) => {
  try {
    const com_id = Number(req.params._id)
        //1: điều chuyển vị trí tài sản
    let filter = {};
    filter.id_cty = com_id
    filter.xoa_dieuchuyen = 0
    filter.dc_type = 0
    let data = await DieuChuyen.aggregate([
      { $match: filter },
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (data[i].id_tai_san != 0) {
        let id_ng_su_dung_tai_san = await TaiSanDangSuDung.findOne({ com_id_sd: com_id, id_ts_sd: data[i].id_tai_san })
        if (id_ng_su_dung_tai_san) data[i].id_ng_su_dung_tai_san = id_ng_su_dung_tai_san.doi_tuong_dang_sd
        else data[i].id_ng_su_dung_tai_san = 0
      }
      if (data[i].id_ng_su_dung_tai_san != 0) {
      ...
      if (data[i].dc_ngay != 0) {
        const newDate = new Date(data[i].dc_ngay * 1000);
        data[i].dc_ngay = `${newDate.getDate()}-${newDate.getMonth() + 1}-${newDate.getFullYear()}`;
      }
      if (data[i].dc_hoan_thanh != 0) data[i].dc_hoan_thanh = new Date(data[i].dc_hoan_thanh * 1000);
      if (data[i].dc_date_create != 0) data[i].dc_date_create = new Date(data[i].dc_date_create * 1000);
      if (data[i].dc_date_delete != 0) data[i].dc_date_delete = new Date(data[i].dc_date_delete * 1000);
      if (data[i].dc_trangthai === 0) data[i].dc_trangthai = "Chờ tiếp nhận";
      if (data[i].dc_trangthai === 1) data[i].dc_trangthai = "Hoàn thành";
      if (data[i].dc_trangthai === 2) data[i].dc_trangthai = "Từ chối tiếp nhận";
      if (data[i].dc_trangthai === 3) data[i].dc_trangthai = "Chờ bàn giao";
    }
    const DCVitriTS_xlsx = [
      ['STT', 
      'Số biên bản', 
      'Ngày điều chuyển', 
      'Trạng thái' , 
  ...
  for (let i = 0; i < data.length; i++) {
      ...
      data[i].dc_trangthai ,
      data[i].dc_vi_tri_tu,
      data[i].dc_vi_tri_den,
      data[i].dc_lydo,
      data[i].ten_ng_thuchien,
      ];
      DCVitriTS_xlsx.push(row);
  }
  quanlytaisanService.excel(DCVitriTS_xlsx, "DieuChuyenViTri", res);
  } catch (error) {
    console.log(error);
    return functions.setError(res, error.message)
  }
}
exports.excelDTSD = async (req, res, next) => {
  try {
    const com_id = Number(req.params._id)
        //1: điều chuyển doi tuong
    let filter = {};
    filter.id_cty = com_id
    filter.xoa_dieuchuyen = 0
    filter.dc_type = 1
  ...
  for (let i = 0; i < data.length; i++) {
      ...
      data[i].dc_ngay,
      data[i].dc_trangthai ,
      data[i].id_nv_dangsudung !== 0 ? data[i].name_nv_dangsudung : data[i].name_pb_dang_sd,
      data[i].id_nv_nhan !== 0 ? data[i].name_nv_nhan  : data[i].name_pb_nhan,
      data[i].dc_lydo,
      data[i].ten_ng_thuchien,
      ];
      DCDoiTuong_xlsx.push(row);
  }
  quanlytaisanService.excel(DCDoiTuong_xlsx, "DieuChuyenDoiTuong", res);
  } catch (error) {
    console.log(error);
    return functions.setError(res, error.message)
  }
}
exports.excelDVQL = async (req, res, next) => {
  try {
    const com_id = Number(req.params._id)
        //1: điều chuyển don vi quan li
    let filter = {};
    filter.id_cty = com_id
    filter.xoa_dieuchuyen = 0
    filter.dc_type = 2
  ...
  for (let i = 0; i < data.length; i++) {
      ...
      data[i].dc_ngay,
      data[i].dc_trangthai ,
      data[i].ten_cty_dang_sd,
      data[i].ten_cty_nhan,
      data[i].dc_lydo,
      data[i].ten_ng_thuchien,
      ];
      DCDonViQL_xlsx.push(row);
  }
  quanlytaisanService.excel(DCDonViQL_xlsx, "DieuChuyenDonViQuanLi", res);
  } catch (error) {
    console.log(error);
    return functions.setError(res, error.message)
  }
  ...
  try {
    ...
    if (loai_bg_dc == 0) {
      ...
      if (this_dc) {
        ...
        const vi_tri_tu = this_dc.vi_tri_dc_tu
        ...
        for (let i = 0; i < taisan_dc.length; i++) {
          ...
          if (check_tsvt_tu) {
            let sl_tu_moi = Number(check_tsvt_tu.tsvt_soluong) + Number(sl_ts)
          ...
          if (this_danhsach_ts_dc.id_nv_dangsudung != 0) {
            ...
            if (query_ts_dang_sd) {
              ...
              if (this_danhsach_ts_dc.id_pb_nhan != 0) {
                ...
                query_this_dt_dd = await TaiSanDaiDienNhan.findOne({
              ...
              if (query_this_dt_dd) {
                let sl_da_cong = Number(query_this_dt_dd.sl_dd_nhan) || 0
                let cong_taisan_dd = await TaiSanDaiDienNhan.updateOne({
                  id_nv_dd_nhan: id_nv_dai_dien,
                  id_cty_dd: id_cty,
                  id_ts_dd_nhan: ts_id,
                }, {
                  sl_dd_nhan: sl_da_cong + Number(sl_ts)
                })
              } else {
        ...
        for (let i = 0; i < taisan_dc.length; i++) {
          ...
          if (query_ts) {
            if (query_ts.ts_so_luong > sl_ts) {
              ...
              let max = await TaiSan.findOne({}, { ts_id: 1 }).sort({ ts_id: -1 }).limit(1).lean() || 0
              let newTS = new TaiSan({
                ts_id: Number(max.ts_id) + 1 || 1,
                id_cty: this_dc.id_cty_nhan,
                id_loai_ts: "chưa cập nhật",
                id_nhom_ts: "chưa cập nhật",
                id_dv_quanly: id_cty,
                id_ten_quanly: id_cty,
                ts_ten: query_ts.ts_ten,
                sl_bandau: sl_ts,
                ts_so_luong: sl_ts,
                soluong_cp_bb: 0,
                ts_gia_tri: query_ts.ts_gia_tri,
                ts_don_vi: query_ts.ts_don_vi,
                ts_vi_tri: "chưa cập nhật",
                ts_trangthai: query_ts.ts_trangthai,
                ts_da_xoa: 0,
                ts_date_create: Date.parse(now) / 1000,
                don_vi_tinh: query_ts.don_vi_tinh,
                ghi_chu: query_ts.ghi_chu,
              })
              await newTS.save()
            }
          }
