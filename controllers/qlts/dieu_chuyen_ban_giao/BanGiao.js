...
exports.list = async (req, res) => {
    try{
        ...
        const id_ban_giao = req.body.id_ban_giao
        ...
        if(id_ban_giao) {
            listConditions["$or"] = [
                {id_ng_thuchien : Number(id_ban_giao) },
                {id_ng_thuhoi : Number(id_ban_giao) },
            ]
        }
        ...
        // end Query
                ...
                { $unwind: { path: "$info", preserveNullAndEmptyArrays: true } },
                    ...
                    {$group : {  
                        "_id": "$id_ng_thuchien",
                        "cp_trangthai": { "$first": "$cp_trangthai" },
                        "ten_ng_thuchien": { "$first": "$info.userName" },
                        "dep_id": { "$first": "$info.inForPerson.employee.organizeDetailId" },
                    }}, 
                    {$skip : skip },
                    {$limit : limit },
                ]) 
                ...
                if(UserAllocation[j].dep_id != 0 ){
                    let depName_cua_nhan_vien = await OrganizeDetail.findOne({ id: UserAllocation[j].dep_id }, { organizeDetailName: 1 })
                    if (depName_cua_nhan_vien) UserAllocation[j].depName_cua_nhan_vien = depName_cua_nhan_vien.organizeDetailName
                    else UserAllocation[j].depName_cua_nhan_vien = "Chưa cập nhật"
                }


                 let numSl_bb = await capPhat.find({id_cty:id_cty,id_ng_thuchien: UserAllocation[j]._id}).countDocuments()
                 UserAllocation[j].numSl_bb = numSl_bb


                let numsl_dtn = await capPhat.find({id_cty:id_cty,id_ng_thuchien: UserAllocation[j]._id, cp_trangthai : 1}).countDocuments()
                 UserAllocation[j].numsl_dtn = numsl_dtn


                let numsl_cn = await capPhat.find({id_cty:id_cty,id_ng_thuchien: UserAllocation[j]._id, cp_trangthai : 0}).countDocuments()
                 UserAllocation[j].numsl_cn = numsl_cn


                let numsl_tc  = await capPhat.find({id_cty:id_cty,id_ng_thuchien: UserAllocation[j]._id, cp_trangthai : 2}).countDocuments()
                ...
                let UserRecall = await ThuHoi.aggregate([
                    ...
                    {$group  : { 
                        "_id": '$id_ng_thuhoi',
                        "thuhoi_trangthai": { "$first": "$thuhoi_trangthai" },
                        "ten_nguoi_thuhoi": { "$first": "$info.userName" },
                        "dep_id": { "$first": "$info.inForPerson.employee.organizeDetailId" },
                    }},
                    {$skip : skip },
                    {$limit : limit },
                ...
                let depName_cua_nhan_vien = await OrganizeDetail.findOne({ id: UserRecall[t].dep_id }, { organizeDetailName: 1 })
                if (depName_cua_nhan_vien) UserRecall[t].depName_cua_nhan_vien = depName_cua_nhan_vien.organizeDetailName
                else UserRecall[t].depName_cua_nhan_vien = "Chưa cập nhật"
                }
                let numSl_bb = await ThuHoi.find({id_cty:id_cty,id_ng_thuhoi: UserRecall[t]._id}).countDocuments()
                UserRecall[t].numSl_bb = numSl_bb
                // listRecall[0].numSl_bb = numSl_bb

                let numsl_dtn = await ThuHoi.find({id_cty:id_cty,id_ng_thuhoi: UserRecall[t]._id, thuhoi_trangthai : 1}).countDocuments()
                UserRecall[t].numsl_dtn = numsl_dtn
                // listRecall[0].numsl_dtn = numsl_dtn

                let numsl_cn = await ThuHoi.find({id_cty:id_cty,id_ng_thuhoi: UserRecall[t]._id, thuhoi_trangthai : 0}).countDocuments()
                UserRecall[t].numsl_cn = numsl_cn

                let numsl_tc  = await ThuHoi.find({id_cty:id_cty,id_ng_thuhoi: UserRecall[t]._id, thuhoi_trangthai : 2}).countDocuments()
                UserRecall[t].numsl_tc = numsl_tc
                }
        let totalCount = dem_bg
