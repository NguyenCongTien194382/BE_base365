...
exports.danhSachTaiBaoMat = async(req, res, next) => {
    try{
        let {page, pageSize, key, dataType,id_ng_nhan_denbu} = req.body;
        if(!page) page = 1;
        ...
        if(id_ng_nhan_denbu) condition.id_ng_nhan_denbu = Number(id_ng_nhan_denbu);
        ...
        if (dataType == 1) {
            ...
            condition.xoa_dx_mat = 0
            condition.mat_trangthai = {$in: [0, 2]};
        } else if (dataType == 2) {
            condition.id_cty = id_cty
            condition.xoa_dx_mat = 0
        ...
        let danhSachMat = await Mat.aggregate([
            ...
            { $project: {
                ...
                "giaTri_taisan": "$TaiSan.ts_gia_tri",
                ...
                "dep_id_ng_nhan_denbu": "$NguoiNhanDenBu.inForPerson.employee.organizeDetailId",               
                
                "id_ng_tao": "$id_ng_tao",
                "ten_ng_tao": "$NguoiTao.userName", 
                "ten_ng_bao_mat": "$NguoiBaoMat.userName", 
                "dep_id": "$NguoiBaoMat.inForPerson.employee.organizeDetailId",
        ...
        for(let i=0; i<danhSachMat.length; i++) {
            ...
            danhSachMat[i].dep_name = mat.ten_ng_tao;
            if(mat.mat_type_quyen == 2) {
                let id_phongban = await OrganizeDetail.findOne({ id: danhSachMat[i].dep_id }, { organizeDetailName: 1 })
                if (id_phongban) danhSachMat[i].dep_name_ng_tao = id_phongban.organizeDetailName
                else danhSachMat[i].dep_name_ng_tao = "Chưa cập nhật"
            }
            danhSachMat[i] = mat;
            if (danhSachMat[i].dep_id_ng_nhan_denbu != 0) {
                let id_phongban = await OrganizeDetail.findOne({ id: danhSachMat[i].dep_id_ng_nhan_denbu }, { organizeDetailName: 1 })
                if (id_phongban) danhSachMat[i].name_pb_ng_nhan_denbu = id_phongban.organizeDetailName
                else danhSachMat[i].name_pb_ng_nhan_denbu = "Chưa cập nhật"
            }
            // if (!danhSachMat[i].ten_ng_duyet) {
            //     let com = await Users.findOne({ _id : data[i].id_ng_duyet })
            //     if (com) danhSachMat[i].ten_ng_duyet = com.userName
            //     else danhSachMat[i].ten_ng_duyet = "Chưa cập nhật"
            // }
        }
    }
...
exports.danhSachChoDenBuVaMat = async(req, res, next) => {
    try{
        ...
        for(let i=0; i<danhSachChoDenBuVaMat.length; i++) {
            ...
            if(mat.mat_type_quyen == 2) {
                let id_phongban = await OrganizeDetail.findOne({ id: mat.id_ng_tao }, { organizeDetailName: 1 })
                if (id_phongban) dep_name = id_phongban.organizeDetailName
                else dep_name = "Chưa cập nhật"
            }
            mat.dep_name_ng_tao = dep_name;
            danhSachChoDenBuVaMat[i] = mat;
            if (danhSachChoDenBuVaMat[i].dep_id_ng_nhan_denbu != 0) {
                let id_phongban = await OrganizeDetail.findOne({ id: danhSachChoDenBuVaMat[i].dep_id_ng_nhan_denbu }, { organizeDetailName: 1 })
                if (id_phongban) danhSachChoDenBuVaMat[i].name_pb_ng_nhan_denbu = id_phongban.organizeDetailName
                else danhSachChoDenBuVaMat[i].name_pb_ng_nhan_denbu = "Chưa cập nhật"
            }
            if (danhSachChoDenBuVaMat[i].id_vi_tri_tai_san != 0) {
                let ten_vi_tri_ts = await ViTri_ts.findOne({ id_vitri: danhSachChoDenBuVaMat[i].id_vi_tri_tai_san })
                if (ten_vi_tri_ts) danhSachChoDenBuVaMat[i].ten_vi_tri_ts = ten_vi_tri_ts.vi_tri
                else danhSachChoDenBuVaMat[i].ten_vi_tri_ts = null
            }
        }
        const total = await functions.findCount(Mat, condition);
    ...
    try{
        ...
        let data = await Mat.aggregate([
            ...
            { $project: {
                ...
                "id_ng_tao": "$id_ng_tao",
                "ten_ng_tao": "$NguoiTao.userName",
                "dep_id_ng_tao": "$NguoiTao.inForPerson.employee.organizeDetailId",
        ...
        if (data) {
            for (let i = 0; i < data.length; i++) {
                if (data[i].dep_id_ng_tao != 0) {
                    let id_phongban = await OrganizeDetail.findOne({ id: data[i].dep_id_ng_tao }, { organizeDetailName: 1 })
                    if (id_phongban) data[i].name_pb_ng_tao = id_phongban.organizeDetailName
                    else data[i].name_pb_ng_tao = "Chưa cập nhật"
                }
            }
        }
        const total = await functions.findCount(Mat, condition);
        return functions.success(res,'get data success',{page, pageSize, total, data})
    }catch(error){
        return functions.setError(res,error.message);
    }
}
...
exports.createMat = async(req, res, next) => {
    try{
        let {ts_mat, giatri_ts, ycdenbu, ngay_mat, hinhthucdb, sl_mat, ngnhandb, nglammat, phantram_nhap, tiendb,quyen_ng_nhan_db,
            day_htdenbu, lydo, giatrimat} = req.body;
        if(ts_mat && ngay_mat && sl_mat && lydo ) {
       
            let id_cty = req.user.data.com_id;
            let id_ng_tao = req.user.data._id;
            let type_quyen = req.user.data.type;
            ngay_mat = new Date(ngay_mat);
            if(day_htdenbu) day_htdenbu = new Date(day_htdenbu);
            ...
            if(ngay_mat <= today) {
                    ...
                    let fields = {
                        ...
                        type_quyen_nhan_db: quyen_ng_nhan_db, 
...
exports.duyet = async(req, res, next) => {
    try{
        let {id_mat, duyet_hinhthuc, duyet_yc, so_tien_nhap, phantram_nhap,day_htdenbu} = req.body;
        let com_id = req.user.data.com_id;
        let id_ng_duyet = req.user.data._id;
        let type_quyen_duyet = req.user.data.type;
        let htdenbu = 0
        if(day_htdenbu) {
            htdenbu = new Date(day_htdenbu);
        }else{
            htdenbu = new Date()
        }
        let phantram = 0;
        ...
        if(duyet_yc == 0) {
            if(duyet_hinhthuc==0) {
                so_tien_duyet = Number(so_tien_nhap);
            }else {
                so_tien_duyet = Number(so_tien_nhap);
        ...
        if(mat) {
            ...
            if(duyet_yc == 1) {
                ...
                let q_doituong_sd_ts = await TaiSanDangSD.findOne({
                    com_id_sd: com_id, 
                    id_ts_sd: id_ts_mat, 
                    $or : [
                        {"id_nv_sd": id_ng_nhan},
                ...
                if(q_doituong_sd_ts) {
                    ...
                    if(sl_dang_sd_update >= 0) {
                        let update_dang_sd = await TaiSanDangSD.findOneAndUpdate({
                            com_id_sd: com_id, 
                            id_ts_sd: id_ts_mat, 
                            $or : [
                            {"id_nv_sd": id_ng_nhan},
                        ...
                        if(update_dang_sd) {
                            let sotien_duyet_cu = mat.so_tien_da_duyet != undefined ? mat.so_tien_da_duyet : 0;
                            let sotien_duyet_moi = Number(sotien_duyet_cu) + so_tien_duyet;
                            let update_mat = await Mat.findOneAndUpdate({mat_id: id_mat, id_cty: com_id}, {
                                ...
                                mat_trangthai: 3,
                                mat_han_ht: Date.parse(htdenbu)/1000,
                                ngay_duyet: ngay_duyet,
                            });
                            if(update_mat) {
                                let id_sd = await functions.getMaxIdByField(QuaTrinhSD, 'quatrinh_id');
                                let fieldsQTSD = {
                                    quatrinh_id: id_sd,
                                    id_ts: id_ts_mat,
                                    id_bien_ban: id_mat,
        ...
        if(datatype == 1) {
            ...
            for(let i=0; i<array_xoa.length; i++) {
                let mat = await Mat.findOneAndUpdate({mat_id: array_xoa[i], id_cty: com_id}, {
                    xoa_dx_mat: 1, 
                    mat_type_quyen_xoa: type_quyen, 
                    mat_id_ng_xoa: id_ng_xoa, 
                    mat_date_delete: date_delete
                }, {new: true});
            }
            return functions.success(res, "Xoa tam thoi thanh cong!");
        }else{
            await Mat.findOneAndUpdate({mat_id: array_xoa, id_cty: com_id}, {
                xoa_dx_mat: 1, 
                mat_type_quyen_xoa: type_quyen, 
                mat_id_ng_xoa: id_ng_xoa, 
                mat_date_delete: date_delete
            }, {new: true});
            return functions.success(res, "Xoa 1 tam thoi thanh cong!");
        }
        }
        if(datatype == 2) {
            if (Array.isArray(array_xoa) === true) {
            ...
            return functions.success(res, "Khoi phuc 1 thanh cong!");
        }
        }
        if(datatype == 3) {
            if (Array.isArray(array_xoa) === true) {
            ...
            await Mat.findOneAndDelete({mat_id: array_xoa, id_cty: com_id});
            return functions.success(res, "Xoa vinh vien 1 thanh cong!");
        }
        }
...
exports.hoanThanh = async(req, res, next) => {
    try{
        ...
        if(id_db && sotien_thanhtoan_nhap && ngay_ht) {
            ...
            if(mat) {
                // if(mat.tien_denbu ? Number(mat.tien_denbu) : 0 <= Number(sotien_thanhtoan_nhap) ){
                //     return functions.setError(res, "Số tiền đền bù phải lớn hơn số tiền thanh toán nhập!");
                // }
                let gt_db_conlai = mat.tien_denbu != undefined ? mat.tien_denbu : 0 - Number(sotien_thanhtoan_nhap);
                let tien_danhan = Number(sotien_thanhtoan_nhap) || 0;
...
exports.ExportExcel = async (req, res, next) => {
    ...
    for(let i=0; i<danhSachMat.length; i++) {
        ...
        if (danhSachMat[i].mat_date_create != 0) {
            const newDate = new Date(danhSachMat[i].mat_date_create * 1000);
            danhSachMat[i].mat_date_create = `${newDate.getDate()}-${newDate.getMonth() + 1}-${newDate.getFullYear()}`;
            }
        if (danhSachMat[i].mat_ngay != 0) {
            const newDate = new Date(danhSachMat[i].mat_ngay * 1000);
            danhSachMat[i].mat_ngay = `${newDate.getDate()}-${newDate.getMonth() + 1}-${newDate.getFullYear()}`;
            }
        if (danhSachMat[i].mat_han_ht != 0) {
            const newDate = new Date(danhSachMat[i].mat_han_ht * 1000);
            danhSachMat[i].mat_han_ht = `${newDate.getDate()}-${newDate.getMonth() + 1}-${newDate.getFullYear()}`;
            }
        if (danhSachMat[i].ngay_thanhtoan != 0) {
            const newDate = new Date(danhSachMat[i].ngay_thanhtoan * 1000);
            danhSachMat[i].ngay_thanhtoan = `${newDate.getDate()}-${newDate.getMonth() + 1}-${newDate.getFullYear()}`;
            }
        if (danhSachMat[i].mat_trangthai === 0) danhSachMat[i].mat_trangthai = "Chờ duyệt";
        if (danhSachMat[i].mat_trangthai === 2) danhSachMat[i].mat_trangthai = "Từ chối";
        if (danhSachMat[i].hinhthucdenbu  === 0) danhSachMat[i].hinhthucdenbu  = "Số tiền";
        if (danhSachMat[i].hinhthucdenbu  !== 0) danhSachMat[i].hinhthucdenbu  = "Chờ duyệt";
    }
}
        //ds mất
        if (dataType == 1) {
            ...
            for (let i = 0; i < danhSachMat.length; i++) {
                const row = [
                i + 1,
                danhSachMat[i].mat_id,
                danhSachMat[i].mat_trangthai ,
                danhSachMat[i].mat_date_create,
                danhSachMat[i].id_taisan,
                danhSachMat[i].ten_taisan,
                danhSachMat[i].mat_soluong,
                danhSachMat[i].loai_taisan,
                danhSachMat[i].ten_ng_tao,
                danhSachMat[i].dep_name_ng_tao,
                danhSachMat[i].mat_ngay,
                danhSachMat[i].mat_lydo,
                ];
                BaoMat_xlsx.push(row);
            }
            quanlytaisanService.excel(BaoMat_xlsx, "DanhSachTSBaoMat", res);
        }
           //ds chờ đền bù
        else if (dataType == 2) {
            const MatChoDenBu_xlsx = [
                ['STT', 
                'Số biên bản', 
                'Mã tài sản',
                'Tên tài sản',
            ...
            for (let i = 0; i < danhSachMat.length; i++) {
                ...
                danhSachMat[i].mat_ngay,
                danhSachMat[i].mat_lydo,
                danhSachMat[i].ten_ng_duyet,
                danhSachMat[i].hinhthucdenbu ,
                danhSachMat[i].tien_denbu,
                danhSachMat[i].ten_ng_nhan_denbu,
                danhSachMat[i].mat_han_ht,
                ];
                MatChoDenBu_xlsx.push(row);
            }
            quanlytaisanService.excel(MatChoDenBu_xlsx, "DanhSachMatChoDenBu", res);
        }
           //ds mất
        else if (dataType == 3) {
            const DaMat_xlsx = [
            ...
            for (let i = 0; i < danhSachMat.length; i++) {
                ...
                danhSachMat[i].ten_ng_duyet,
                danhSachMat[i].hinhthucdenbu ,
                danhSachMat[i].so_tien_da_duyet ,
                danhSachMat[i].sotien_danhan,
                danhSachMat[i].ten_ng_nhan_denbu,
                danhSachMat[i].mat_han_ht,
                danhSachMat[i].ngay_thanhtoan,
                ];
                DaMat_xlsx.push(row);
            }
            quanlytaisanService.excel(DaMat_xlsx, "DanhSachTSDaMat", res);
        }
    } catch (error) {
      console.log(error);
      return functions.setError(res, error.message)
