...
exports.ExportExcel = async (req, res, next) => {
    try {
      ...
      let condition = { id_cty: com_id, donvi_xoa: 0 };
      let danhSachDonViCongSuat = await DonViCongSuat.find(condition).sort({ id_donvi: -1 });
      const DonViCongSuat_xlsx = [
        ['STT', 'Mã đơn vị', 'Tên đơn vị đo công suất', 'Mô tả']
    ];
    for (let i = 0; i < danhSachDonViCongSuat.length; i++) {
        const row = [
        i + 1,
        danhSachDonViCongSuat[i].id_donvi,
        danhSachDonViCongSuat[i].ten_donvi,
        danhSachDonViCongSuat[i].mota_donvi,
        ];
        DonViCongSuat_xlsx.push(row);
    }
    quanlytaisanService.excel(DonViCongSuat_xlsx, "danhSachDonViCongSuat", res);
    } catch (error) {
      console.log(error);
...
exports.add_do_csuat = async (req, res) => {
    try {
        ...
        if (chon_ngay && fnc.checkDate(chon_ngay)) chon_ngay = quanlytaisanService.convertTimestamp(chon_ngay);
        else chon_ngay = 0
...
exports.update_theodoi_cs = async (req, res) => {
    try {
        ...
        if (id && day_update && cs_thuc) {
            ...
            if (fnc.checkDate(day_update)) day_update = quanlytaisanService.convertTimestamp(day_update);
            ...
            if (theodoi) {
                //xu li cong tgian
                let newDate = 0
                let checkNgayNhap = Number(theodoi.nhap_ngay) || 0 
                if(checkNgayNhap !== 0) {
                     newDate = day_update + (checkNgayNhap * 24 * 60 * 60 * 1000)
                }
                let update = await TheoDoiCongSuat.findOneAndUpdate({
                    ...
                    chon_ngay: theodoi.chon_ngay !== 0 ? theodoi.chon_ngay : newDate,
                    cs_gannhat: cs_thuc
                })
