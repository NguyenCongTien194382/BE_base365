        ...
        if (q_this_sc) {
            ...
            if (quyen_ng_sd == 2) {
                ...
                if (q_taisan_doituong) {
                    let sl_ts_cu = Number(q_taisan_doituong.sl_dang_sd);
                    let update_sl = sl_ts_cu + sl_sc;
...
exports.listBBDangSuaChua = async (req, res) => {
    try {
        ...
        const sc_ng_thuchien = req.body.sc_ng_thuchien
        ...
        if (sc_ng_thuchien) conditions.sc_ng_thuchien = Number(sc_ng_thuchien)
        const data1 = await SuaChua.aggregate([
            ...
            { $unwind: { path: "$infoNV", preserveNullAndEmptyArrays: true } },
            {
                $project: {
                    ...
                    "dep_id": "$infoNV_ng_thuchien.inForPerson.employee.organizeDetailId",
        ...
        for (let i = 0; i < data1.length; i++) {
            if(data1[i].dep_id != 0 ) {
                ...
                if (id_phongban) data1[i].ten_pb_ng_thuc_hien = id_phongban.organizeDetailName
                else data1[i].ten_pb_ng_thuc_hien = "Chưa cập nhật"
            }
            if (data1[i].id_vi_tri_tai_san != 0) {
                let ten_vi_tri_ts = await ViTri_ts.findOne({ id_vitri: data1[i].id_vi_tri_tai_san })
                if (ten_vi_tri_ts) data1[i].sc_ts_vitri = ten_vi_tri_ts.vi_tri
                else data1[i].sc_ts_vitri = null
            }
...
exports.listAllSuaChua = async (req, res) => {
    try {

        const id_cty = req.user.data.com_id
        const sc_id = req.body.sc_id
        const id_ts = req.body.id_ts
        const sc_ng_thuchien = req.body.sc_ng_thuchien
        const page = Number(req.body.page) || 1
        const pageSize = Number(req.body.pageSize) || 10
        const skip = (page - 1) * pageSize
        ...
        conditions.sc_da_xoa = 0
        conditions.id_cty = id_cty
        if (id_ts) conditions.suachua_taisan = Number(id_ts)
        if (sc_id) conditions.sc_id = Number(sc_id)
        if (sc_ng_thuchien) conditions.sc_ng_thuchien = Number(sc_ng_thuchien)
        const data1 = await SuaChua.aggregate([
            { $match: conditions },
        ...
        data.push({ list: data1 })
        let totalCount = await SuaChua.countDocuments(conditions)
        ...
        if (isNaN(xoa_vinh_vien) || isNaN(type_quyen)) {
            ...
            if (xoa_vinh_vien == 0) {
                ...
                let xoa_sua_chua = await SuaChua.findOneAndUpdate({ id_cty: com_id, sc_id: { $in: xoa } },
                    {
                        sc_da_xoa: 1,
                        sc_type_quyen_xoa: type_quyen, sc_id_ng_xoa: id_ng_xoa
                    });
                return fnc.success(res, "xoa thanh cong");
            } else if (xoa_vinh_vien == 2) {
                //khoi phuc
...
exports.addSuaChua = async (req, res) => {
    try {
        let { id_ts, sl_sc, trangthai_sc, loai_bb, sc_quyen_sd, ng_sd, ngay_sc, vitri_ts, dv_sc, dia_chi_nha_cung_cap,
            ngay_dukien, hoanthanh_sc, chiphi_dukien, chiphi_thucte, nd_sc, ng_thuc_hien, dia_diem_sc, } = req.body;
        ...
        if (loai_bb == 0) {
            ...
            await new_SuaChua.save();
                if (sc_quyen_sd == 2) {
                    ...
                    let q_taisan_doituong = await TaiSanDangSuDung.findOne({ com_id_sd: com_id, id_nv_sd: ng_sd, id_ts_sd: id_ts });
                    if (q_taisan_doituong) {
                        let sl_ts_cu = Number(q_taisan_doituong.sl_dang_sd) || 0;
                        let update_sl = sl_ts_cu - Number(sl_sc);
            ...
            return fnc.success(res, "tạo thành công sửa chữa tài sản đã cấp phát", { new_SuaChua })
           ...
           //sua chua tai san chua cap phat
            ...
            if (sc_quyen_sd == 1) {
                ...
                if (ts) {
                let sl_ts_cu = Number(ts.ts_so_luong) || 0;
                let update_sl = 0;
                if (sl_ts_cu > sl_sc) {
            ...
            return fnc.success(res, "tạo thành công sửa chữa tài sản chưa cấp phát", { new_SuaChua_1 })
        }
    } catch (error) {
        console.log(error)
        return fnc.setError(res, error.message);
    ...
    try {
        ...
        if (sc_id) {
            ...
            for (let i = 0; i < data1.length; i++) {
                ...
                if (data1[i].id_vi_tri != 0) {
                    let ten_vi_tri_ts = await ViTri_ts.findOne({ id_vitri: data1[i].id_vi_tri })
                    if (ten_vi_tri_ts) data1[i].vi_tri = ten_vi_tri_ts.vi_tri
                    else data1[i].vi_tri = null
                }
                if (data1[i].sc_ngay != 0) data1[i].sc_ngay = new Date(data1[i].sc_ngay * 1000);
                if (data1[i].sc_ngay_nhapkho != 0) data1[i].sc_ngay_nhapkho = new Date(data1[i].sc_ngay_nhapkho * 1000);
                if (data1[i].sc_date_duyet != 0) data1[i].sc_date_duyet = new Date(data1[i].sc_date_duyet * 1000);
...
exports.CanSCExcel = async (req, res, next) => {
    try {
    ...
    for (let i = 0; i < data1.length; i++) {
        ...
        if (data1[i].sc_date_create != 0) {
            const newDate = new Date(data1[i].sc_date_create * 1000);
            data1[i].sc_date_create = `${newDate.getDate()}-${newDate.getMonth() + 1}-${newDate.getFullYear()}`;
        }
        if (data1[i].sc_ngay_hong != 0) {
            const newDate = new Date(data1[i].sc_ngay_hong * 1000);
            data1[i].sc_ngay_hong = `${newDate.getDate()}-${newDate.getMonth() + 1}-${newDate.getFullYear()}`;
        }
        if (data1[i].sc_ngay_nhapkho != 0) {
            const newDate = new Date(data1[i].sc_ngay_nhapkho * 1000);
            data1[i].sc_ngay_nhapkho = `${newDate.getDate()}-${newDate.getMonth() + 1}-${newDate.getFullYear()}`;
        }
        if (data1[i].sc_trangthai  === 0) data1[i].sc_trangthai  = "Chờ duyệt";
        if (data1[i].sc_trangthai  === 1) data1[i].sc_trangthai  = "Đang sửa chữa";
        if (data1[i].sc_trangthai  === 2) data1[i].sc_trangthai  = "Từ chối";
        if (data1[i].sc_trangthai  === 3) data1[i].sc_trangthai  = "Hoàn thành";
    }
    ...
    for (let i = 0; i < data1.length; i++) {
        ...
        i + 1,
        data1[i].sc_id,
        data1[i].sc_trangthai ,
        data1[i].sc_date_create ,
        data1[i].ma_tai_san,
        data1[i].ten_tai_san,
        data1[i].sl_sc,
        data1[i].doi_tuong_sd,
        data1[i].sc_ts_vitri,
        data1[i].sc_ngay_hong,
        data1[i].sc_ngay_nhapkho ? data1[i].sc_ngay_nhapkho :  'Chưa cập nhật' ,
        data1[i].sc_noidung,
        ];
        CanSC_xlsx.push(row);
    }
    quanlytaisanService.excel(CanSC_xlsx, "DanhSachCanSuaChua", res);
    } catch (error) {
      console.log(error);
      return functions.setError(res, error.message)
    }
  }
exports.DangSCExcel = async (req, res, next) => {
    try {
      const com_id = Number(req.params._id)
       let conditions = {}
        conditions.sc_da_xoa = 0
        ...
        for (let i = 0; i < data1.length; i++) {
            ...
            if (data1[i].sc_ngay != 0) {
                const newDate = new Date(data1[i].sc_ngay * 1000);
                data1[i].sc_ngay = `${newDate.getDate()}-${newDate.getMonth() + 1}-${newDate.getFullYear()}`;
            }
            if (data1[i].sc_dukien != 0) {
                const newDate = new Date(data1[i].sc_dukien * 1000);
                data1[i].sc_dukien = `${newDate.getDate()}-${newDate.getMonth() + 1}-${newDate.getFullYear()}`;
            }
            if (data1[i].sc_ngay_nhapkho != 0) data1[i].sc_ngay_nhapkho = new Date(data1[i].sc_ngay_nhapkho * 1000);
            if (data1[i].sc_date_duyet != 0) data1[i].sc_date_duyet = new Date(data1[i].sc_date_duyet * 1000);
            if (data1[i].sc_date_create != 0) data1[i].sc_date_create = new Date(data1[i].sc_date_create * 1000);
    ...
    for (let i = 0; i < data1.length; i++) {
        ...
        data1[i].ten_tai_san,
        data1[i].sl_sc,
        data1[i].sc_chiphi_dukien ,
        data1[i].sc_ngay ,
        data1[i].sc_dukien,
        data1[i].sc_noidung,
        ];
        DangSC_xlsx.push(row);
    }
    quanlytaisanService.excel(DangSC_xlsx, "DanhSachDangSuaChua", res);
    } catch (error) {
      console.log(error);
      return functions.setError(res, error.message)
    }
  }
exports.DaSCExcel = async (req, res, next) => {
    try {
      const com_id = Number(req.params._id)
       let conditions = {}
        conditions.sc_da_xoa = 0
        ...
        for (let i = 0; i < data1.length; i++) {
            ...
            if (data1[i].sc_hoanthanh != 0) {
            const newDate = new Date(data1[i].sc_hoanthanh * 1000);
            data1[i].sc_hoanthanh = `${newDate.getDate()}-${newDate.getMonth() + 1}-${newDate.getFullYear()}`;
            }
            if (data1[i].sc_ngay_nhapkho != 0) data1[i].sc_ngay_nhapkho = new Date(data1[i].sc_ngay_nhapkho * 1000);
            if (data1[i].sc_date_duyet != 0) data1[i].sc_date_duyet = new Date(data1[i].sc_date_duyet * 1000);
            if (data1[i].sc_date_create != 0) data1[i].sc_date_create = new Date(data1[i].sc_date_create * 1000);
    ...
    for (let i = 0; i < data1.length; i++) {
        ...
        data1[i].sl_sc,
        data1[i].sc_chiphi_dukien ,
        data1[i].sc_chiphi_thucte ,
        data1[i].sc_ngay ,
        data1[i].sc_dukien,
        data1[i].sc_hoanthanh,
        data1[i].sc_noidung,
        ];
        DaSC_xlsx.push(row);
    }
    quanlytaisanService.excel(DaSC_xlsx, "DanhSachDaSuaChua", res);
    } catch (error) {
      console.log(error);
      return functions.setError(res, error.message)
    }
