...
const updateAndSaveHistory = async(emp_id_old, customer_id, adm_bophan, last_status_called, last_time_called, id_cus_from, emp_id_new, cus_from = 'tv365') => {
    try {
        console.log("updateAndSaveHistory", emp_id_old, customer_id, adm_bophan, last_status_called, last_time_called, id_cus_from, emp_id_new, cus_from)
        let customer = await Customer.findOne({ cus_id: Number(customer_id) }).lean();
        if (customer) {
            console.log("Có tồn tại tài khoản");
            ...
            if (response && response.data) {
                ...
                console.log("Không có cuộc gọi nào")
                let from = cus_from;
                ...
                // Site khác tạm thời bỏ qua vì ko có khách
...
const customer_not_call = async() => {
    try {
        ...
        while (true) {
            ...
            const time = 60000 * 10; // 2 phút quét 1 lần
            ...
            const search = {
                ...
                //updated_at: { $gte: last_time_called_stone }, // Lấy những nhà cập nhật gần nhất 
